...
class EventSummaryReportViewSet(viewsets.GenericViewSet):
    ...
    def export_summary_pdf(self, request, pk=None):
        try:
            ...
            male_count = participant_counts["male_count"] or 0
            female_count = participant_counts["female_count"] or 0
            ...
            event_images = event.event_docs.filter(
                doc_type__icontains="image",
            )[:2]
            ...
            for img in event_images:
                try:
                    if hasattr(img.file_path, "path"):
                        image_path = img.file_path.path
                    else:
                        image_path = img.file_path

                    images_base64.append(get_image_base64(image_path))

                except Exception as e:
                    logger.warning("Error loading event image: %s", e)
            ...
            html_string = render_to_string(
                "event/event_summary_report.html",
                report_data,
            )

            pdf_buffer = generate_pdf_sync(html_string)

            logger.info("Summary PDF for event %s generated", event.event_id)

            return build_pdf_response(pdf_buffer, filename)

        except Exception as e:
            logger.exception(
                "Error generating summary PDF for event %s: %s",
                pk,
                str(e),
            )
